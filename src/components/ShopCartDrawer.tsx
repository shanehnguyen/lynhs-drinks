  ...
  return (
    <>
      <button
        ...
        className="relative flex h-11 w-11 items-center justify-center rounded-full border-2 border-coffee bg-warmwhite text-ink"
      ...
      >
        ...
        {items.length > 0 && (
          <span className="absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full bg-thai text-[11px] font-bold text-warmwhite">
            {items.length}
          </span>
        )}
      </button>
      ...
      {open && mounted && createPortal(
        <div className="fixed inset-0 z-[60]">
          <button
            aria-label="Close"
            className="absolute inset-0 bg-coffee/40"
            onClick={() => setOpen(false)}
          />
          <div className="absolute right-0 top-0 flex h-full w-full max-w-sm flex-col bg-cream p-6 shadow-[-8px_0_0_0_rgba(43,27,18,0.15)]">
            <div className="flex items-center justify-between">
              <h2 className="text-2xl text-ink">My Picks</h2>
              <button
                onClick={() => setOpen(false)}
                className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-coffee text-ink"
            ...
            {items.length === 0 ? (
              ...
              <div className="mt-6 flex-1 space-y-4 overflow-y-auto">
                {items.map((item) => (
                  <div
                    key={item.id}
                    className="relative rounded-lg border-[3px] border-coffee bg-warmwhite p-4 shadow-[4px_4px_0_0_#2B1B12]"
                  >
                    <button
